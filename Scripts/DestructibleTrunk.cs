...
public class DestructibleTrunk : MonoBehaviour, IDestructible
{
    ...
    public void Initialize()
    {
        ...
        faceMesh = new Mesh();
        faceMesh.vertices = new Vector3[] {new Vector3(0f, 0f, 0f), new Vector3(0f, height, 0f), new Vector3(width, height, 0f), new Vector3(width, 0f, 0f)};
        faceMesh.uv = new Vector2[] {new Vector2f(0f, 0f), new Vector2f(0f, 1f), new Vector2f(1f, 1f), new Vector2f(1f, 0f)};
        faceMesh.normals = new Vector3[] {new Vector3(0f, 0f, -1f), new Vector3(0f, 0f, -1f), new Vector3(0f, 0f, -1f), new Vector3(0f, 0f, -1f)};
        faceMesh.triangles = new int[] {0, 1, 3, 3, 1, 2};
    ...
    private void UpdateBlockBounds(int x, int y)
    {
        ...
        BlockSimplification.currentLowerPoint = new Vector2i {x = lx * blockSizeScaled, y = ly * blockSizeScaled};

        BlockSimplification.currentUpperPoint = new Vector2i {x = ux * blockSizeScaled, y = uy * blockSizeScaled};
    }
    ...
    public void UpdateEdgeMesh()
    {
        ...
        for (int i = 0; i < blocks.Length; i++)
        {
            ...
            System.Array.Copy(blocks[i].subVertices,
                0,
                vertices,
                vertexIndex,
                vertexCount);
            System.Array.Copy(blocks[i].subTexCoords,
                0,
                texCoords,
                vertexIndex,
                vertexCount);
            System.Array.Copy(blocks[i].subNormals,
                0,
                normals,
                vertexIndex,
                vertexCount);
            System.Array.Copy(blocks[i].subTriangles,
                0,
                triangles,
                triangleIndex,
                triangleCount);
